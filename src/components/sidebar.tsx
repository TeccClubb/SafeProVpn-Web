...
const Sidebar = () => {
  ...
  const links: NavLink[] = [
    ...
    { name: 'Billing', href: '/Dashboard/Account-biling-history', icon: <CreditCard className="w-5 h-5 mr-2" /> },
    ...
    {links
      ...
      .map((link) => (
        <div key={link.name} className="w-full flex justify-center md:justify-start">
          <Link
            href={link.href}
            className={`w-full flex items-center px-4 py-2 text-base md:text-lg transition-all duration-200 ${
              pathname === link.href
